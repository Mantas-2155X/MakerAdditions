    ...
    public static class Hooks
    {
        ...
        public static IEnumerable<CodeInstruction> ChaControl_SetAccessoryPos_ChangeLimit(IEnumerable<CodeInstruction> instructions)
        {
...

            {
                ...
                foreach (var index in items.Select(item => il.IndexOf(item)).ToList())
                {
                    if (index <= 0)
                    {
                        KK_MakerAdditions.Logger.LogMessage("Failed transpiling 'ChaControl_SetAccessoryPos_ChangeLimit' f1 index not found!");
                        KK_MakerAdditions.Logger.LogWarning("Failed transpiling 'ChaControl_SetAccessoryPos_ChangeLimit' f1 index not found!");
                        return il;
                    }
...

            {
                ...
                foreach (var index in items.Select(item => il.IndexOf(item)).ToList())
                {
                    if (index <= 0)
                    {
                        KK_MakerAdditions.Logger.LogMessage("Failed transpiling 'ChaControl_SetAccessoryPos_ChangeLimit' Clamp index not found!");
                        KK_MakerAdditions.Logger.LogWarning("Failed transpiling 'ChaControl_SetAccessoryPos_ChangeLimit' Clamp index not found!");
                        return il;
                    }
        ...
        public static IEnumerable<CodeInstruction> ChaControl_SetAccessoryRot_ChangeLimit(IEnumerable<CodeInstruction> instructions)
        {
            ...
            foreach (var index in items.Select(item => il.IndexOf(item)).ToList())
            {
                if (index <= 0)
                {
                    KK_MakerAdditions.Logger.LogMessage("Failed transpiling 'ChaControl_SetAccessoryRot_ChangeLimit' Conv_I4 index not found!");
                    KK_MakerAdditions.Logger.LogWarning("Failed transpiling 'ChaControl_SetAccessoryRot_ChangeLimit' Conv_I4 index not found!");
                    return il;
                }
            ...
            foreach (var index in items1.Select(item => il.IndexOf(item)).ToList())
            {
                if (index <= 0)
                {
                    KK_MakerAdditions.Logger.LogMessage("Failed transpiling 'ChaControl_SetAccessoryRot_ChangeLimit' Repeat index not found!");
                    KK_MakerAdditions.Logger.LogWarning("Failed transpiling 'ChaControl_SetAccessoryRot_ChangeLimit' Repeat index not found!");
                    return il;
                }
